...
type Props = {
  ...
  onCancel: () => void;
  onConfirm: (payload: { dataUrl: string; fileName: string }) => void | Promise<void>;
};
...
export default function AnnotateOverlay({
  ...
  }, [deleteSelected, handleRedo, handleUndo, onCancel, open]);
  ...
  const handleConfirm = useCallback(async () => {
    ...
    if (!naturalSize || !renderUrl) return;
    setConfirming(true);
    try {
      ...
      const blob: Blob = await new Promise((resolve, reject) => {
        canvas.toBlob((b) => (b ? resolve(b) : reject(new Error("blob_failed"))), "image/png", 0.92);
      });
      const url = URL.createObjectURL(blob);
      const fileName = normalizeFileName(imageFileName);
      await onConfirm({ dataUrl: url, fileName });
    } finally {
      setConfirming(false);
    }
  }, [imageFileName, isConfirming, items, naturalSize, onConfirm, renderUrl]);
        ...
        >
          ...
          >
            ...
            <button
              ...
              title="完成"
              className={cn(
                "ml-2 flex h-10 w-10 shrink-0 items-center justify-center rounded-full shadow-sm transition",
            ...
            >
              <ForwardedIconComponent
                name={isConfirming ? "Loader2" : "Check"}
                className={cn("h-5 w-5", isConfirming && "animate-spin")}
              />
            </button>
          </div>
        </div>
