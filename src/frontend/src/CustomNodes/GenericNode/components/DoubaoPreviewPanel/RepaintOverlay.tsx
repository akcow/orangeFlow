...
const RepaintOverlay = forwardRef<RepaintOverlayHandle, Props>(function RepaintOverlay(
  ...
  const drawStroke = useCallback(
    (from: { x: number; y: number } | null, to: { x: number; y: number }) => {
      ...
      const line = (ctx: CanvasRenderingContext2D, strokeStyle: string | CanvasPattern) => {
        ctx.save();
