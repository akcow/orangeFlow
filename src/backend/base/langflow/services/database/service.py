...
class DatabaseService(Service):
    ...
    @staticmethod
    def _is_ignorable_migration_diff(message: str) -> bool:
        normalized = " ".join(str(message or "").split())
        if "modify_type" not in normalized:
            return False
        if "TIMESTAMP(timezone=True), DateTime()" not in normalized:
            return False
        blocking_tokens = (
            "add_table",
            "remove_table",
        ...
        return not any(token in normalized for token in blocking_tokens)
    ...
    def _run_migrations(self, should_initialize_alembic, fix) -> None:
        ...
        with self.alembic_log_path.open("w", encoding="utf-8") as buffer:
            ...
            try:
                ...
                if self._is_ignorable_migration_diff(str(exc)):
                    logger.warning(
                        "Ignoring autogenerate diffs that only reflect PostgreSQL timestamp type rendering differences."
                    )
                    return
                if not fix:
